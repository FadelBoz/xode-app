...
export function CirclesIcon({ style, colors }: ConcentricCirclesIconProps) {
    const colo = useThemeColors();

    // Assign colors or use defaults from the theme
    const circleColors = {
        innerCircle: colors?.innerCircle || colo.card ,
        middleCircle: colors?.middleCircle || colo.card ,
        outerMiddleCircle: colors?.outerMiddleCircle || colo.cardForeground,
        outerCircle: colors?.outerCircle || colo.cardForeground,
    };

    return (
        <View style={[styles.container, style]}>
            <Svg width="186" height="186" viewBox="0 0 196 196" fill="none">
                <Circle
                    cx="98"
                    cy="96"
                    r="52"
